...
public class GunshipController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (wasLeftMousePressed)
        {
            ...
            GameObject leftCanoonball = Instantiate(cannonballPrefab, leftCannon.position, Quaternion.identity);
            RigidbodyLeft = leftCanoonball.GetComponent<Rigidbody2D>();
            RigidbodyLeft.AddForce(leftCannon.right * 10f, ForceMode2D.Impulse);
        ...
        if (wasRightMousePressed)
        {
            ...
            GameObject rightCanoonball = Instantiate(cannonballPrefab, rightCannon.position, Quaternion.identity);
            RigidbodyLeft = rightCanoonball.GetComponent<Rigidbody2D>();
            RigidbodyLeft.AddForce(rightCannon.right * 10f, ForceMode2D.Impulse);
